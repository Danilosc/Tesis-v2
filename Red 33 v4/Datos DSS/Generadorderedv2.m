...
for i=1:1:length(Tipo_Nodo)
    if Tipo_Nodo(i)=='consumo'
        ...
        %fprintf(lineas, '%s%s %s%s%s%s %s%s %s%s %s%s %s%s %s%s %s\n','new load.Load', string(i), 'bus1=', Nodo_carga(i),'.',string(confas(i)), 'phases=', string(fasesn(i)), 'conn=', string(conn(i)),'kV=12.6 kW=', string(1), 'kvar=', string(1), 'model=1 yearly=perfilcarga', string(j), 'status=variable');
    ...
    elseif Tipo_Nodo(i)=='generador'
        ...
        k=k+1;
%     elseif contains(Tipo_Nodo(i),'EVCS')== 1
%         EVid = char(Tipo_Nodo(i));
%         fprintf(lineas, '%s%s%s%s %s%s%s%s %s%s %s%s %s%s %s%s %s%s%s%s %s \n','new load.LoadEV', string(Nodo_carga(i)), '.', EVid(5), 'bus1=', Nodo_carga(i),'.', string(confas(i)), 'phases=',string(fasesn(i)),'conn=', string(conn(i)), 'kV=12.6 kW=', string(kw_nom(i)), 'kvar=', string(kvar_nom(i)), 'model=1 yearly=perfilEVCS', string(Nodo_carga(i)), '.', EVid(5) , 'status=variable'); 
%     elseif contains(Tipo_Nodo(i),'Strg')== 1
%         batid = char(Tipo_Nodo(i));
%         fprintf(lineas, '%s%s%s%s %s%s%s%s %s%s %s%s %s%s %s%s %s%s%s%s %s%s \n','new storage.EV', string(Nodo_carga(i)), '.', batid(5), 'bus1=', Nodo_carga(i),'.', string(confas(i)), 'phases=',string(fasesn(i)),'conn=', string(conn(i)), 'kV=12.6 kW=', string(kw_nom(i)), 'kvar=', string(kvar_nom(i)), 'model=1 yearly=perfilstrg', string(Nodo_carga(i)), '.', batid(5) , 'dispmode=follow %Charge=100 %Discharge=100 %EffCharge=90 %EffDischarge=90 %reserve=20 kwhrated=', string(kw_nom(i))); 
    end
...
%     elseif contains(Tipo_Nodo(i),'EVCS')== 1
%         EVid = char(Tipo_Nodo(i));
%         fprintf(monitor,'%s%s%s%s %s%s%s%s %s\n','New monitor.MonitorpowEV',string(Nodo_carga(i)), '.',string(EVid(5)),'element=load.LoadEV',string(Nodo_carga(i)), '.', string(EVid(5)),'Terminal=1 mode=1 ppolar=no');
%         fprintf(monitor,'%s%s%s%s %s%s%s%s %s\n','New monitor.MonitorvolEV',string(Nodo_carga(i)), '.',string(EVid(5)),'element=load.LoadEV',string(Nodo_carga(i)), '.', string(EVid(5)),'Terminal=1 mode=0 ppolar=no');
%     elseif contains(Tipo_Nodo(i),'Strg')== 1
%         batid = char(Tipo_Nodo(i));
%         fprintf(monitor,'%s%s%s%s %s%s%s%s %s\n','New monitor.Monitorpowbat',string(Nodo_carga(i)), '.',string(batid(5)),'element=storage.EV',string(Nodo_carga(i)), '.', string(batid(5)),'Terminal=1 mode=1 ppolar=no');
%         fprintf(monitor,'%s%s%s%s %s%s%s%s %s\n','New monitor.Monitorvolbat',string(Nodo_carga(i)), '.',string(batid(5)),'element=storage.EV',string(Nodo_carga(i)), '.', string(batid(5)),'Terminal=1 mode=0 ppolar=no');
     end
end
